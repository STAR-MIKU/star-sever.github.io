...
export class MP4MuxingRecorder {
            ...
            if (packet.type === 'configuration') {
                switch (this.videoMetadata.codec) {
                    case ScrcpyVideoCodecId.H264: {
                        ...
                        // configuration handled by mp4-muxer automatically
                        break;
                    }
    ...
    private addAudioChunk(chunk: ScrcpyDataPacket) {
        ...
        this.muxer.addAudioChunkRaw(chunk.data, 'key', timestamp, 0); // 添加duration参数
    }
    ...
    public start() {
        ...
        try {
            ...
            const options: ConstructorParameters<typeof Muxer>[0] = {
                target: new ArrayBufferTarget(),
                firstTimestampBehavior: 'offset',
                fastStart: false,
                video: {
                    codec: MP4VideoCodecNameMap[this.videoMetadata.codec!],
            ...
            if (this.audioCodec) {
                options.audio = {
                    codec: MP4AudioCodecNameMap[this.audioCodec.mimeType!],
                    sampleRate: 48000,
                    numberOfChannels: 2,
                };
            }

            this.muxer = new Muxer(options as any);
    ...
    public stop() {
        ...
        try {
            // 尝试finalize，如果失败则直接使用当前buffer
            try {
                this.muxer.finalize()!;
            } catch (finalizeError) {
                console.warn('Warning: muxer.finalize() failed, using available buffer:', finalizeError);
            }
            ...
            // 使用新的下载函数
            downloadFile(buffer, fileName, 'video/mp4');
