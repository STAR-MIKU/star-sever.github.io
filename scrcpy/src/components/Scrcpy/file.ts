import { WrapReadableStream, WritableStream, ReadableStream, MaybeConsumable } from '@yume-chan/stream-extra';
...
export function createFileStream(file: File, onProgress?: (progress: number) => void): ReadableStream<MaybeConsumable<Uint8Array>> {
    let totalRead = 0;

    // 创建直接兼容 MaybeConsumable<Uint8Array> 的 ReadableStream
        ...
        start(controller) {
            ...
            reader.onload = () => {
                if (reader.result instanceof ArrayBuffer) {
                    ...
                    totalRead += reader.result.byteLength;
                    ...
                    // 直接将 Uint8Array 作为 MaybeConsumable 发送
                    controller.enqueue(chunk as MaybeConsumable<Uint8Array>);
                }
